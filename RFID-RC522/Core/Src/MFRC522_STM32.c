...
void MFRC522_Init(MFRC522_t *dev) {
    ...
    // IMPORTANT: make CalcCRC use ISO14443A preset 0x6363.
    // ModeReg reset value is 0x3F (CRCPreset=0b11 -> 0xFFFF), which breaks Select/Auth.
    MFRC522_WriteReg(dev, PCD_ModeReg, 0x3D);        // CRCPreset=01 (6363h)
